 ...
 *
 * Disassembly of DSDT-orig.aml, Fri Apr 15 12:53:47 2022
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x00011258 (70232)
 *     Revision         0x02
 *     Checksum         0x9E
 *     OEM ID           "_ASUS_"
...
DefinitionBlock ("", "DSDT", 2, "_ASUS_", "Notebook", 0x00000013)
{
                ...
                Device (LCDD)
                {
                    ...
                    Method (MINB, 0, NotSerialized)
                    {
                        If (ARDY)
                        {
                            ...
                            Store (DerefOf (Index (^^^LPCB.EC.PWAC, Local1)), Local0)
                            Or (TCHE, 0x08, TCHE)
                            Or (ASLC, 0x08, ASLC)
                            Store (Local0, PFMB)
                            Or (PFMB, 0x0100, PFMB)
                    ...
                    Method (_BCL, 0, NotSerialized)  // _BCL: Brightness Control Levels
                    {
                        ...
                        Else
                        {
                            ...
                            While (LLess (Local0, 0x0B))
                            {
                                ...
                                Store (DerefOf (Index (^^^LPCB.EC.PWAC, Local3)), Local1)
                    ...
                    Method (_BQC, 0, NotSerialized)  // _BQC: Brightness Query Current
                    {
                        ...
                        Else
                        {
                            ...
                            Store (Multiply (DerefOf (Index (^^^LPCB.EC.PWAC, Local2)), 0x64), Local3)
    ...
    Scope (_SB.PCI0.LPCB.EC)
    {
        ...
        Method (BIFA, 0, NotSerialized)
        {
            If (ECAV ())
            {
                If (BSLF)
                {
                    Store (B1B2(B1SX,B1SK), Local0)
                }
                Else
                {
                    Store (B1B2(B0SX,B0SK), Local0)
                }
            }
            Else
            {
            ...
            If (LLessEqual (Local2, 0x03E8))
            {
                ...
                If (LEqual (DerefOf (Index (Local0, Zero)), Zero))
                {
                    ...
                    If (LEqual (Arg0, RDWD))
                    {
                        ...
                        Store (B1B2(T2B1,T2B2), Index (Local0, 0x02))
                    }
    ...
    Scope (_SB.PCI0.LPCB.EC)
    {
        ...
        {
            /* 0000 */  0x0E, 0x21, 0x2F, 0x40, 0x4E, 0x5F, 0x6D, 0x7B,
        ...
        Method (TACH, 1, Serialized)
        {
            If (ECAV ())
            {
                Switch (Arg0)
                {
                    Case (Zero)
                    {
                        Store (B1B2(AH01,AH02), Local0)
                        Break
                    }
                    Case (One)
                    {
                        Store (B1B2(AH11,AH12), Local0)
                        Break
                    }
                    Default
                    {
    ...
    Scope (_SB)
    {
        Device (LID)
        {
            ...
            {
                Store (One, Local0)
                Store (^^PCI0.LPCB.EC.RPIN (0x46), Local0)
                If (LEqual (Local0, Ones))
                {
                    Store (One, Local0)
                }
            ...
            If (LNotEqual (\_SB.PTAB, Zero))
            {
                ...
                If (LNotEqual (EVNT, Zero))
                {
                    ...
                    If (LNotEqual (ECH2, Zero))
                    {
                        If (LEqual (EVNT, One))
                        {
                            \_SB.PCI0.LPCB.EC.ST95 (0x7F, One)
                            Store (HP02, AUX2)
                            Store (LP01, AUX1)
                        }
                        ...
                        If (LEqual (EVNT, 0x10))
                        {
                            \_SB.PCI0.LPCB.EC.ST95 (0x7F, Zero)
                            Store (HP02, AUX2)
                            Store (HP01, AUX1)
                        }
                        ...
                        If (LEqual (EVNT, 0x08))
                        {
                            \_SB.PCI0.LPCB.EC.ST95 (0x7F, 0x02)
                            Store (LP02, AUX2)
                            Store (LP01, AUX1)
                        }
    ...
    Scope (\)
    {
        ...
        {
            SGPL (0x0F, One, Zero)
            \_SB.PCI0.LPCB.EC.SPIN (0x95, Zero)
        }
        ...
        Method (PRJW, 1, Serialized)
        {
            ...
            {
                \_SB.PCI0.LPCB.EC.SPIN (0x35, One)
            }
        ...
        {
            Return (\_SB.PCI0.LPCB.EC.RPIN (0x46))
        }
        ...
        Method (ACPS, 0, Serialized)
        {
            Store (\_SB.PCI0.LPCB.EC.RPIN (0x24), Local0)
            XOr (Local0, One, Local0)
            Return (Local0)
        }
